...
import { ClientStore, StoredBooking } from "@/lib/client-store";
...
export function CheckoutClient({ bookingId }: { bookingId: string }) {
  ...
  useEffect(() => {
    // Try to load booking
    const b = ClientStore.getBooking(bookingId);
    if (b) {
      setBooking(b);
      setLoading(false);
    } else {
      // Create fallback preview booking
      const fallback = ClientStore.createBooking("st-dune-imax-1", ["seat-E-6", "seat-E-7"]);
      setBooking(fallback);
      setLoading(false);
    }
  }, [bookingId]);
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    // Simulate declined card if ending in 0002
    if (cardNumber.replace(/\s/g, "").endsWith("0002")) {
      setErrorMessage("Your card was declined. Please try another card or payment method.");
      setIsProcessing(false);
      return;
    }

    try {
      const { ticket } = await ClientStore.confirmPayment(booking.id);
      router.push(`/tickets/preview`);
    } catch (err: any) {
